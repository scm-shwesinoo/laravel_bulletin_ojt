                ...
                <div class="card">
                    ...
                    <div class="card-body">
                            ...
                            <form action="{{ route('postlist') }}" method="GET" class="w-auto d-flex p-0">
                                <div class="input-group mx-2">
                                    <input type="text" class="form-control" type="text" name="search"
                                        value="{{ request('search') }}" id="search-keyword">
                                    <span class="input-group-text">
                                        <a class="header-btn text-secondary" href="{{ url('post/list') }}"><i
                                                class="fa-solid fa-xmark"></i></a>
                                    </span>
                                </div>
                                <button class="btn btn-outline-primary mx-2 w-auto search-btn"
                        ...
                        <form action="{{ route('postlist') }}" method="get" class="mb-3">
                            ...
                            <select class="form-select d-inline w-auto" name="page_size" id="post_page_size">
                                    ...
                                    @foreach ($postList as $post)
                                        <tr>
                                            <td style="width: 20%;"><a href="#"
                                                    onclick="Post.prototype.showPostDetail({{ $post }}, {{ $post->createdBy }}, {{ $post->updatedBy }})"
                                                    data-bs-toggle="modal"
                                                    data-bs-target="#post-detail-modal">{{ $post->title }}</a></td>
                                                ...
                                                <td style="width: 15%;">
                                                    @if (auth()->user()->id == $post->created_user_id)
                                                        <a type="button" class="btn btn-primary"
                                                            href="/post/edit/{{ $post->id }}">Edit</a>
                                                        <button type="button"
                                                            onclick="Post.prototype.showDeleteConfirm({{ $post }})"
                                                            class="btn btn-danger post-delete" data-bs-toggle="modal"
                                                            data-bs-target="#post-delete-modal">Delete</button>
                                                    @endif
                                                </td>
                                            @endif
                                        </tr>
                        ...
                        @if (count($postList) > 0)
                            ...
                            <div class="modal fade" id="post-delete-modal" tabindex="-1"
